...
if (!isset($_SESSION["accessLV"]) || $_SESSION["accessLV"] !== true) {
    $tmp = $_SESSION["listSender"];
    header("location: $tmp");
}
...
<html lang="en">
    <head>
        <title><?php echo $_GET['listName'] ?></title>
        <!-- Required meta tags -->
        <meta charset="utf-8" />
        <meta
            name="viewport"
    ...
    <body style="background-color: #000933;">
        <header>
            <?php $current_page = $_SESSION['listSender']; include 'navbar.php'; ?>
        </header>
        <main>
            <div id="dati" data-lista='<?php echo json_encode($lista); ?>'></div>
            <h2 style="color: white; padding: 30px; text-align: center;"><?php echo $_GET['listName'] ?></h2>
        ...
        <script>
            ...
            const list = <?php echo json_encode($_SESSION['listToView']); ?> ;
            ...
            list.forEach((item) => {
                fetch(baseurl + item)
                    ...
                    .then((data) => {
                        ...
                        const card = createCard(data, list);
